...
let sonarqubeVersionPromise;

module.exports = {

  onInit: function(config, dependencies) {
    sonarqubeVersionPromise = fetchSonarQubeVersion(config, dependencies);
  },

  onRun: function(config, dependencies, job_callback) {

    Promise.all( [ sonarqubeVersionPromise, fetchListOfProjects(config, dependencies) ] )
      .then(function([ sqVersion, projects ] ) {
        return fetchProjectsMeasures(config, dependencies, sqVersion, projects);
      })
...
function fetchProjectsMeasures(config, dependencies, sqVersion, projects) {
  ...
  // with recursion, only 1 req at a time, diluting the load on SonarQube
  return new Promise(
    function(resolve, reject) {
  ...
  function queryOneProject(resolve, reject, projectIndex) {
    if (projectIndex < projects.length) {
      const path = util.format(measureQuery, projects[projectIndex].k);
      const options = computeRequestOptionsForSQ(config, path);
      dependencies.request(options, function(err, response, body) {
        if (err) {
          reject(err);
...
module.exports.fetchSonarQubeVersion = fetchSonarQubeVersion
function fetchSonarQubeVersion(config, dependencies) {
  return new Promise(
    function(resolve, reject) {
      const path = '/api/server/version';
      const options = computeRequestOptionsForSQ(config, path);
      dependencies.request(options, function(err, response, body) {
        if (err) {
          reject(err);
          ...
          // attempt to decode the SQ version number,
          const elts = body.match(/(\d+)\.(\d+)(?:\.(\d+)(?:\.(\d+))?)?(?:-(\w+))?/)
          if (elts) {
            resolve({
              full: elts[0],
              major: Number.parseInt(elts[1]),
              minor: Number.parseInt(elts[2]),
              patch: elts[3] ? Number.parseInt(elts[3]) : null,
              build: elts[4] ? Number.parseInt(elts[4]) : null,
              qualifier: elts[5]
            });
          } else {
            resolve({
